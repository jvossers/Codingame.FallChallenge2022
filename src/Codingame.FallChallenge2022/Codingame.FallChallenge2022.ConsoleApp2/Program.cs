...
class Player
{
    static void Main(string[] args)
    {
        ...
        while (true)
        {
            ...
            var oppPatches = ctx.Patches.Cast<Patch>().Where(p => p.Owner == 0);
            var keepGoing = myPatches.Any(p => p.GetNeighbours().Any(n => n.ScrapAmount > 0 && (n.Owner == 0 || n.Owner == -1)));

            if (keepGoing)
            {
                var attackingDirection = myPatches.Min(p => p.X) < oppPatches.Min(p => p.X) ? Direction.Right : Direction.Left;
                ctx.QueueCommand($"MESSAGE {attackingDirection}");

                #region build

                using (var scope = new StopwatchScope("build"))
                {
                    if (ctx.MyRecyclerCount < 6 && ctx.RoundCounter < 25)
                    {
                        if (attackingDirection == Direction.Right)
                        {
                            var attackPatch = myPatches.Where(p => p.CanBuild == 1 && p.X >= (ctx.Width / 2) - 1).OrderByDescending(p => p.X).FirstOrDefault();
                            if (attackPatch != null)
                            {
                        ...
                        if (attackingDirection == Direction.Left)
                        {
                            var attackPatch = myPatches.Where(p => p.CanBuild == 1 && p.X <= (ctx.Width / 2)).OrderBy(p => p.X).FirstOrDefault();
                            if (attackPatch != null)
                            {
                                ctx.QueueCommand($"BUILD {attackPatch.X} {attackPatch.Y}");
                                ctx.MyMatter = ctx.MyMatter - 10;
